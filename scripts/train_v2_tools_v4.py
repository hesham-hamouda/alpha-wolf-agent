#!/usr/bin/env python3
"""
V2_Tools continue — AGGRESSIVE xformers disable via monkey patching.

Strategy: Intercept xformers import BEFORE peftModel tries to use it.
Force xformers to be a dummy module that returns None for everything.
"""
...
# PRE-IMPORT: Create a dummy xformers module that does nothing
class _DummyXformersModule:
    """Dummy replacement for xformers that disables all operations."""
    def __getattr__(self, name):
        return _DummyXformersModule()
    def __call__(self, *args, **kwargs):
        return None

class _DummyXformersOps:
    def __getattr__(self, name):
        return _DummyXformersModule()

# Install dummy xformers BEFORE any other imports
xformers_dummy = types.ModuleType('xformers')
xformers_dummy.__getattr__ = lambda name: _DummyXformersModule()
sys.modules['xformers'] = xformers_dummy

xformers_ops = types.ModuleType('xformers.ops')
xformers_ops.__getattr__ = lambda name: _DummyXformersOps()
sys.modules['xformers.ops'] = xformers_ops

xformers_ops_fmha = types.ModuleType('xformers.ops.fmha')
sys.modules['xformers.ops.fmha'] = xformers_ops_fmha

print("[INFO] xformers monkey-patched to dummy")
...
OUTPUT_DIR = "/workspace/v2_tools_v4"
...
def main():
    print("=" * 70)
    print("V2_TOOLS CONTINUE v4 — Monkey-patched xformers disable")
    print("=" * 70)

    print(f"\n[1/4] Tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)

    print(f"\n[2/4] Base + checkpoint-500 adapter (xformers dummy'd)...")
